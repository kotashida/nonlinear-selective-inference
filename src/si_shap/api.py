...
def _validate_data(X, y, k_select, sigma):
    ...
    if not isinstance(k_select, (int, np.integer)) or not 1 <= k_select <= X.shape[1]:
...
def selective_inference(
    ...
    *,
    ...
    if not isinstance(ais_seed, (int, np.integer)):
    ...
    observed = _validate_selection_result(
        resolved_selector.select(X, y, k_select), X.shape[1], k_select
    )
    ...
    for position, feature in enumerate(observed.selected_features, start=1):
        ...
        def is_selected(z):
            ...
            if z not in cache:
                ...
                candidate_result = _validate_selection_result(
                    resolved_selector.select(X, candidate, k_select),
                    X.shape[1],
                    k_select,
                )
